...
from glean.schema.codepp.types import (
    ppEntity,
)
...
class CodePpEntity(InnerGleanSchemaPredicate):
  ...
  def build_angle(__env: Dict[str, R], define: ast.Expr, undef: ast.Expr, include_: ast.Expr) -> Tuple[str, Struct]:
    return f"code.pp.Entity.1 {{ { ', '.join(filter(lambda x: x != '', [angle_for(__env, define, 'define'), angle_for(__env, undef, 'undef'), angle_for(__env, include_, 'include_')])) or '_' } }}", ppEntity
