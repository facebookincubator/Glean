...
factToAngle (KeyFact _ (Item outV inVs{-ranges-} fileId Nothing)) = [] <$ do
  ...
  case mResultSet of
    Nothing -> -- don't know result set, so log the edge from definition to def/file
...
emitHovers fileId ids = do
  ...
  if V.null hoverBodies
    then pure Nothing
    else return $ Just $ Predicate "lsif.DefinitionHover.1" $ V.toList hoverBodies
...
predicateId name id_ facts = pure [Predicate name [object [factId id_, key facts ]]]
