...
insertProject (Id n) =
  modify' (\e -> e { openProjects = Set.insert (fromIntegral n) (openProjects e) })
insertDocument (Id n) =
  modify' (\e -> e { openDocuments = Set.insert (fromIntegral n) (openDocuments e) })
...
insertInResultSet (Id n) resultSetId  = modify' $ \e -> e { rangeToResultSetMap =
    ...
    Just ReferenceType | prop == Just References -> do
      ...
      let xrefFacts :: [[Pair]]
          xrefFacts =
            ...
            | inV <- V.toList inVs
            , Just resultSetId <- pure (lookupIn env rangeToResultSetMap inV)
            , Just targetFileId <- pure (lookupIn env resultSetToFileMap resultSetId)
            , Just targetDefId <- pure (lookupIn env resultSetToDefMap resultSetId)
            ]
      if null xrefFacts
        then pure []
        else pure [
