...
produceDecl name maybeTy ctx = case ctx of
  ValBind RegularBind _ _ ->
    case maybeTy of
      Nothing -> return () -- happens for foreign imports
      Just ty -> Glean.makeFact_ @Hs.ValBind $ Hs.ValBind_key name ty
  Decl FamDec _ ->
  ...
  PatternBind{} ->
    case maybeTy of
      Nothing -> fail "missing type for PatternBind"
      Just ty -> Glean.makeFact_ @Hs.PatBind $ Hs.PatBind_key name ty
  TyVarBind{} ->
