...
DEFINE_string(
    ownership_dump,
    "",
    "PATH where ownership data will be dumped as JSON (optional)");
...
struct Config {
  ...
  Config(int argc, char** argv) {
    ...
    if (!FLAGS_dump.empty()) {
      ...
      should_log = false;
      sender = fileWriter(
          FLAGS_dump_every != 0 ? fmt::format("{}.0", FLAGS_dump) : FLAGS_dump);
...
struct SourceIndexer {
  ...
  void flush() {
    ...
    batch =
        std::make_unique<Batch<SCHEMA>>(config.schema.get(), FLAGS_fact_cache);
  }
...
int main(int argc, char** argv) {
  ...
  if (!FLAGS_dry_run) {
    ...
    config.logger("clang/flush").log([&]() { indexer.flush(); });
  }
