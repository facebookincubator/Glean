...
import Glean.Glass.Base (GleanPath (GleanPath), SymbolRepoPath (..))
...
toGleanPath
  rp@SymbolRepoPath (Glass.RepoName "react") (Glass.Path path) =
    GleanPath ("react/" <> path)
toGleanPath SymbolRepoPath {..} = GleanPath path
