...
import qualified GHC.Unit.Types as GHC (mkModule, fsToUnit)
import qualified GHC.Data.FastString as GHC (FastString, mkFastStringByteString)
import qualified GHC.Types.SrcLoc as GHC (noSrcSpan)
...
toIfaceExtName n = do
  Hs.Name_key occ mod sort <- Glean.keyOf n
...
typeOfDeclaration (Hs.Declaration_val val) = do
  k <- Glean.keyOf val
  return (Just (Hs.valBind_key_ty k))
typeOfDeclaration (Hs.Declaration_patBind val) = do
  k <- Glean.keyOf val
  return (Just (Hs.patBind_key_ty k))
