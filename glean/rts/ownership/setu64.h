...
inline uint32_t upper(__m256i value) {
  ...
  if (a < 64) {
    return static_cast<uint32_t>(255 - a);
  }
  a = _lzcnt_u64(p[2]);
  if (a < 64) {
    return static_cast<uint32_t>(191 - a);
  }
  a = _lzcnt_u64(p[1]);
  if (a < 64) {
    return static_cast<uint32_t>(127 - a);
  }
  return static_cast<uint32_t>(63 - _lzcnt_u64(p[0]));
}
...
/**
 * A memory-efficient representation of sets of uint64_t. This is quite similar
 ...
 * measure.
 *
 * We split the uint64_t key space into blocks of 256 values - the upper 56
 ...
 * block, we store a 64-bit block header in the the header array. The header
 * contain the 56-bit block id and an 8-bit control byte (`Hdr`) which says what
...
class SetU64 {
 public:
  ...
  struct Hdr {
    ...
    static Hdr sparse(uint64_t id, uint8_t len) {
      return {id, len, Sparse};
    }

    static Hdr dense(uint64_t id) {
      return {id, 0, Dense};
    }

    static Hdr full(uint64_t id) {
      return {id, 0, Full};
    }
    ...
    bool before(uint64_t id) const {
      return value < (id << 8);
    }

   private:
    Hdr(uint64_t id, uint8_t len, Type type) {
      value = (id << 8) | (uint64_t(len) << 2) | static_cast<uint64_t>(type);
    }
  ...
  SetU64(const SetU64& other, copy_capacity_tag);

  bool operator==(const SetU64& other) const;
  bool operator!=(const SetU64& other) const {
    return !(*this == other);
  }
  ...
  lower_bound(const_iterator start, const_iterator finish, uint64_t id);
  ...
  static SetU64 from(const std::set<uint64_t>& set) {
    SetU64 setu64;
    for (auto elt : set) {
      setu64.append(elt);
    }
   ...
   */
  static const SetU64*
  merge(SetU64& result, const SetU64& left, const SetU64& right);
    ...
    for (auto& block : *this) {
      ...
      switch (block.hdr.type()) {
        case SetU64::Hdr::Sparse: {
          for (uint64_t i = 0; i < block.hdr.sparseLen(); i++) {
            f(id | block.sparse[i]);
          }
          break;
        }
  ...
  static std::set<uint64_t> to(const SetU64& set) {
    std::set<uint64_t> out;
    set.foreach([&](uint64_t elt) { out.insert(elt); });
    return out;
  }
  ...
  MutableEliasFanoList toEliasFano(uint64_t upper) const;
  static SetU64 fromEliasFano(const EliasFanoList& list);

  static void dump(SetU64&);
 ...
 private:
  ...
  void append(uint64_t id, Bits256 w);
