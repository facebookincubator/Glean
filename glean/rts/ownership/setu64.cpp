...
bool SetU64::Block::operator==(const SetU64::Block& other) const {
  if (hdr != other.hdr) {
    return false;
  }
  switch (hdr.type()) {
    case SetU64::Hdr::Sparse:
      return std::memcmp(&*sparse, &*other.sparse, hdr.sparseLen()) == 0;
    case SetU64::Hdr::Dense:
...
bool SetU64::Block::includes(const SetU64::Block& other) const {
  ...
  switch (hdr.type()) {
    case SetU64::Hdr::Sparse:
      return other.hdr.type() == SetU64::Hdr::Sparse &&
          std::includes(
                 sparse,
...
void SetU64::reserve(Sizes sizes) {
  hdrs.reserve(sizes.hdrs);
  dense.reserve(sizes.dense);
  sparse.reserve(sizes.sparse);
}

void SetU64::shrink_to_fit() {
  hdrs.shrink_to_fit();
  dense.shrink_to_fit();
  sparse.shrink_to_fit();
}

size_t SetU64::size() const {
  size_t s = 0;
  for (auto& block : *this) {
...
void SetU64::append(const_iterator start, const_iterator finish) {
...
std::tuple<
    const SetU64*,
    const SetU64*,
    SetU64::const_iterator,
    SetU64::const_iterator>
skipSubset(const SetU64* l, const SetU64* r) {
  auto left = l->begin();
  auto left_end = l->end();
  auto right = r->begin();
  ...
  while (left != left_end && right != right_end) {
    if (left.hdrs->id() < right.hdrs->id()) {
      left = SetU64::lower_bound(left, left_end, right.hdrs->id());
...
void SetU64::append(uint64_t id, Bits256 w) {
  if (w == Bits256::all()) {
...
void SetU64::appendMerge(SetU64::Block left, SetU64::Block right) {
  struct Local {
...
void SetU64::appendMerge(
    SetU64::const_iterator left,
    SetU64::const_iterator left_end,
    SetU64::const_iterator right,
    SetU64::const_iterator right_end) {
  while (left != left_end && right != right_end) {
    if (left.hdrs->id() < right.hdrs->id()) {
      ...
      right = SetU64::lower_bound(right, right_end, left.hdrs->id());
      append(prev, right);
    } else {
...
const SetU64*
SetU64::merge(SetU64& result, const SetU64& left, const SetU64& right) {
  if (&left == &right) {
    return &left;
  } else {
...
SetU64::MutableEliasFanoList SetU64::toEliasFano(uint64_t upperBound) const {
  auto max = this->upper();
  CHECK_LT(max, upperBound);
  size_t size = this->size();
  folly::compression::EliasFanoEncoder<uint64_t, uint64_t> encoder(size, max);

  VLOG(5) << "upper=" << upperBound << ", size=" << size;
  foreach([&](uint64_t elt) { encoder.add(elt); });
  return encoder.finish();
}

SetU64 SetU64::fromEliasFano(const EliasFanoList& list) {
  SetU64 set;
  auto reader =
      EliasFanoReader<folly::compression::EliasFanoEncoder<uint64_t, uint64_t>>(
          list);
  while (reader.next()) {
...
void SetU64::dump(SetU64& set) {
  for (auto& block : set) {
    auto id = block.hdr.id() << 8;
    switch (block.hdr.type()) {
      case SetU64::Hdr::Sparse: {
        for (uint8_t i = 0; i < block.hdr.sparseLen(); i++) {
