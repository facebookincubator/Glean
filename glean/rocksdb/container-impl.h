...
struct Iterator {
    ...
    void seek_first() { iter->SeekToFirst(); }
    void seek_last() { iter->SeekToLast(); }
    void seek_key(folly::ByteRange key) { iter->Seek(slice(key)); }
    void seek_key_lower(folly::ByteRange key) { iter->SeekForPrev(slice(key)); };

    void next() { iter->Next(); }
    void prev() { iter->Prev(); }

    bool valid() { return iter->Valid(); }

    folly::ByteRange key() { return byteRange(iter->key()); }
    folly::ByteRange value() { return byteRange(iter->value()); }
...
struct ContainerImpl final : Container {
  ...
  bool readData(folly::ByteRange key, std::function<void(folly::ByteRange)> f) override {
