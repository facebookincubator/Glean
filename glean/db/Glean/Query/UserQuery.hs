...
userQueryImpl
  ...
  , Thrift.QuerySyntax_ANGLE <- Thrift.userQueryOptions_syntax opts = do
    ...
    -- TODO: we would like to avoid re-processing the query when
    -- there's a continuation.  It's only needed because we need
    -- result type of the query for encoding the results.
    (compileTime, _, query) <- timeIt $
    ...
    let
      ...
      irDiag =
        [ "ir:\n" <> Text.pack (show (pretty (qiQuery query)))
        | Thrift.queryDebugOptions_ir debug ]

      bytecodeDiag sub =
        [ "bytecode:\n" <> (
            Text.unlines $ disassemble "Query" $ compiledQuerySub sub)
        | Thrift.queryDebugOptions_bytecode debug ]

    details@PredicateDetails{..} <- case qiReturnType query of
      Angle.Record
          [ Angle.FieldDef _ (Angle.Predicate (PidRef pid ref))
          , Angle.FieldDef _ keyTy
          , Angle.FieldDef _ valTy ] -> do
        details <-
          case IntMap.lookup (fromIntegral (fromPid pid)) predicatesById of
        ...
        when (not (Text.null userQuery_predicate)) $
...
userQueryImpl
  ...
  Thrift.UserQuery{..} = do
      ...
      userQueryTerm query cont = do
        ...
        let
          ...
          limits
            ...
            where pids = getExpandPids query
          ...
          case cont of
            Just ucont ->
              restartCompiled schemaInventory stack (Just predicatePid) limits
                (Thrift.userQueryCont_continuation ucont)
            Nothing -> do
              gens <- case toGenerators schema stored details query of
                Left err -> throwIO $ Thrift.BadQuery err
                Right r -> return r
              bracket (compileQuery gens) (release . compiledQuerySub)
    ...
    let
      ...
      cont = Thrift.userQueryOptions_continuation opts
    ...
    case runExcept $ parseQuery schema opts details pat of
      ...
      Right (MatchTerm (NestedPred _ Nothing (Just term))) ->
        userQueryTerm term cont
      Right (MatchTerm (NestedPred _ Nothing Nothing)) ->
        userQueryTerm (Ref Wildcard) cont
...
mkUserQueryCont :: ByteString -> Fid -> Thrift.UserQueryCont
mkUserQueryCont cont nextId = hashUserQueryCont $ Thrift.UserQueryCont
