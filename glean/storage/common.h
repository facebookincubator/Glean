...
template <typename T> inline folly::ByteRange bytesOf(const T &x) {
  return folly::ByteRange(reinterpret_cast<const unsigned char *>(&x),
                          sizeof(x));
}

template <typename T> inline T fromBytes(const folly::ByteRange slice) {
...
///
template <typename C> struct DatabaseCommon : Database {
  ...
  getSetIterator(DatabaseCommon<C> &db);
  ...
  std::unique_ptr<rts::OwnershipUnitIterator> getOwnershipUnitIterator() override;
...
template <typename C>
std::unique_ptr<rts::OwnershipSetIterator>
DatabaseCommon<C>::getSetIterator(DatabaseCommon<C> &db) {
  struct SetIterator : rts::OwnershipSetIterator {
    ...
    folly::Optional<std::pair<UsetId, SetExpr<const OwnerSet *>>>
    get() override {
      if (!initial) {
        iter.next();
      } else {
    ...
    std::pair<size_t, size_t> sizes() const override { return {first_, size_}; }
...
template <typename C> uint32_t DatabaseCommon<C>::nextUnitId() {
...
folly::Optional<std::string> DatabaseCommon<C>::getUnit(uint32_t unit_id) {
  ...
  if (container_.get(C::Family::ownershipUnitIds, key.byteRange(),
                     [&](auto val) { r = binary::mkString(val); })) {
    return r;
  } else {
    return folly::none;
  }
...
void DatabaseCommon<C>::addOwnership(
  ...
  for (const auto &set : ownership) {
    ...
    binary::Output key;
    key.nat(unit_id);
    key.nat(unit_id < first_unit_id + ownership_unit_counters.size()
  ...
  if (new_count > 0) {
    ...
    writer.put(C::Family::admin, bytesOf(AdminId::NEXT_UNIT_ID),
...
template <typename C>
void putOwnerSet(C &container, typename C::Writer &writer, UsetId id, SetOp op,
                 const OwnerSet &set) {
  binary::Output key;
  key.nat(id);
  binary::Output value;
...
void DatabaseCommon<C>::storeOwnership(ComputedOwnership &ownership) {
  ...
  if (ownership.facts_.size() > 0) {
    ...
    auto hasOwner = [&](EncodedNat& key) -> bool {
      bool r = false;
      container_.get(C::Family::factOwners, key.byteRange(),
                     [&](auto) { r = true; });
      return r;
    };
    ...
    for (auto uset : def.newSets_) {
      ...
      uset->exp.set.foreach ([&](uint32_t elt) {
        ...
        if (newelt >= next_uset_id) {
          rts::error("set id out of range: {} {} {}", newelt, next_uset_id,
                     elt);
        }
        s.insert(newelt);
      });
...
template <typename C> void DatabaseCommon<C>::cacheOwnership() {
  factOwnerCache_.enable(container_);
}

template <typename C> void DatabaseCommon<C>::prepareFactOwnerCache() {
  FactOwnerCache::prepare(container_);
}

template <typename C> UsetId DatabaseCommon<C>::getOwner(Id id) {
...
void DatabaseCommon<C>::FactOwnerCache::enable(C &container) {
  ...
  if (!container.get(
          C::Family::factOwnerPages, binary::byteRange(INDEX_KEY),
          [&](auto val) {
...
UsetId DatabaseCommon<C>::FactOwnerCache::lookup(
    const DatabaseCommon<C>::FactOwnerCache::Page &page, Id id) {
  ...
  if (page.factIds[low] <= ix) {
    ...
    rts::error("missing lower bound, ix={} low={} prefix={}", ix, low,
               id.toWord() >> PAGE_BITS);
  }
}

template <typename C>
std::optional<UsetId> DatabaseCommon<C>::FactOwnerCache::getOwner(C &container,
                                                                  Id id) {
...
void DatabaseCommon<C>::FactOwnerCache::prepare(C &container) {
  ...
  writer.put(C::Family::factOwnerPages, binary::byteRange(INDEX_KEY),
             folly::ByteRange(reinterpret_cast<const uint8_t *>(index.data()),
                              index.size() * sizeof(UsetId)));

  t.logFormat("{} index entries, {} populated, {} orphans", index.size(),
              populated, orphaned);

  // record the number of orphaned facts, this will be fetched by ownershipStats
  writer.put(C::Family::admin, bytesOf(AdminId::ORPHAN_FACTS),
             bytesOf(orphaned));
...
//
template <typename C> struct StoredOwnership : Ownership {
  explicit StoredOwnership(DatabaseCommon<C> *db) : db_(db) {}

  UsetId nextSetId() override { return db_->next_uset_id; }
  ...
  OwnershipStats getStats() override { return db_->getOwnershipStats(); }
