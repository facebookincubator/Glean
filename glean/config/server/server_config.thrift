// @generated SignedSource<<d20532fdab66d95de87ee75b693a0981>>
...
struct Retention {
  ...
  // Exclude dbs with the following poperties.
  // Opposite of required_properties
  9: Properties excluded_properties = {};

  // If set, only the N oldest complete DBs per UTC calendar day
  // are eligible for retention. Keeping the oldest (rather than the most
  // recent) means newly indexed DBs do not evict the DBs that long-running
  // automation jobs are still referencing. Combine with `retain_at_least`,
  // `retain_at_most` and `delete_if_older` to bound the total number of DBs
  // kept on the server.
  10: optional i32 retain_per_day;
}
...
struct Backup {
  ...
  1: string location = "manifold-xdb:xdb.glean_catalog,glean_databases/nodes/dev-backup";
...
struct DatabaseBackupPolicy {
  ...
  3: string location = "manifold-xdb:xdb.glean_catalog,glean_databases/nodes/dev-backup";
...
struct Config {
  ...
  // If db_rocksdb_cache_to_mem_ratio is set, the value is ignored
  ...
  // If true, check schema IDs in batches against the schema of the
  // DB. This catches a schema mismatch between the writer and the
  ...
  // rocksdb_cache/total_memory_capacity ratio (between 0 and 1)
  ...
  // If true, produce RocksDB backups as a Checkpoint (an openable DB
  // snapshot, tarred under a top-level `db/` directory) instead of a
  // BackupEngine backup (`backup/`). The checkpoint restore path renames
  // the snapshot into place, avoiding the second full copy that drives
  // query-server file-cache OOMs. RocksDB-only; a no-op for LMDB. This
  // gates the backup producer (write/indexer tier); restoring `db/`
  // tarballs is already supported fleet-wide.
  42: bool db_backup_use_checkpoint = true;

  // If true, the write server backs up local Incomplete (still-writable) DBs
  // to durable storage on SIGTERM and restores + reclaims them on startup
  // (Tupperware preemption resilience), and the janitor floors
  // retention.delete_incomplete_if_older at 72h so an in-progress DB is not
  // reaped mid-write. Runtime no-op unless the server runs under exactly one
  // SMC tier (so off-TW / Chronos write servers are unaffected). This is the
  // live kill-switch for the feature: flip to false and ship to disable
  // without a redeploy. Enabled for write tiers in glean.write.common.cinc.
  43: bool backup_incomplete_on_shutdown = false;
}
